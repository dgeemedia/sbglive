...
// The interactive product page (size/colour picking, 360° viewer, add to cart).
// The server fetches the product and passes it in as `initialProduct`, so the page's HTML arrives
// complete — name, price, description and photos are there for search engines and for the first paint,
// instead of being an empty shell that JavaScript fills in later.
'use client'
...
export default function ProductView({ slug, initialProduct }: { slug: string; initialProduct: Product | null }) {
  ...
  useEffect(() => {
    if (initialProduct) return // the server already gave us the product — nothing to fetch
    // Fallback only: if the server couldn't load it (a momentary Sanity error), fetch it here instead
    let cancelled = false
    setLoading(true)
